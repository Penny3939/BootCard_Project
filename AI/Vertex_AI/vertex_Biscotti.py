...
# 設定你的資料
service=os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = './tibame.json'
storage_client = storage.Client()
project='hale-acumen-383307'
location='us-central1'
model_id='1412909825087504384'
model_name='predictBiscotti_2'
bucket_name='speech-ai101-bucket'
blob_name='Biscotti_sales.csv'
endpoint_id='3220104519895482368'
filename=f'gs://{bucket_name}/{blob_name}'
client = aiplatform_v1.services.prediction_service.PredictionServiceClient()

# 讀取資料
row_data = []
with open('./Biscotti_sales.csv', 'r') as f:
    reader = csv.reader(f)
    for row in reader:
        row_data.append(row)
...
instance_json = [{"lstm_18_input":instance.tolist()}]
print(instance_json)
# 在網路上測試可以使用:
...
with open('Biscotti_data.json', 'w') as f:
...
upload_blob('speech-ai101-bucket', 'Biscotti_data.json', 'Biscotti_prediction.json')
...
# 讀取 JSON 數據
with open('./Biscotti_data.json', 'r') as f:
    data = json.load(f)
...
plt.title('Biscotti Prediction')
plt.xlabel('Date')
plt.ylabel('Quantity')
plt.grid(True)
for x, y in zip(df.index, df['Value']):
    plt.text(x, y, str(y))
plt.savefig('./Predict_Biscotti.png')

upload_blob('speech-ai101-bucket', 'Predict_Biscotti.png', 'Predict_Biscotti.png')