...
model_id='5883154667717787648'
model_name='predictPastry'
bucket_name='speech-ai101-bucket' 
blob_name='Pastry_sales.csv'
endpoint_id='5793577455402876928'
...
# 讀取資料
row_data = []
with open('./Pastry_sales.csv', 'r') as f:
    reader = csv.reader(f)
    for row in reader:
        row_data.append(row)
...
instance_json = [{"lstm_68_input":instance.tolist()}]
# print(instance)
...
with open('Pastry_data.json', 'w') as f:
...
upload_blob('speech-ai101-bucket', 'Pastry_data.json', 'Pastry_prediction.json')
...
# 讀取 JSON 數據
with open('./Pastry_data.json', 'r') as f:
    data = json.load(f)
...
plt.title('Pastry Prediction')
plt.xlabel('Date')
plt.ylabel('Quantity')
plt.grid(True)
for x, y in zip(df.index, df['Value']):
    plt.text(x, y, str(y))
plt.savefig('Predict_Pastry.png')

upload_blob('speech-ai101-bucket', 'Predict_Pastry.png', 'Predict_Pastry.png')